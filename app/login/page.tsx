...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Determine if it's admin or provider login based on email
      const isAdmin = email.includes('admin') || email.includes('m3allam.ma') || email.includes('m3allamconnect.ma');
      
      // Use AuthContext login method
      await login(email, password, isAdmin ? 'admin' : 'provider');

      // Successful login - redirect to dashboard
      window.location.href = isAdmin ? "/admin/dashboard" : "/provider/dashboard";
    } catch (error) {
      setError("Email ou mot de passe incorrect.");
    }
  };
